...
FontsRenderer::FontsRenderer(camera* cam, const font_ressource* f)
{
    this->cam = cam;
    this->uid = f->uid;
    for (int i = 0; i < f->fonts.size(); i++)
    {
        font_textures.push_back(Load_Texture(*f->fonts[i]->font_image, cam->renderer));
    }
}
...
void FontsRenderer::render_text(int x, int y, int h, int w, int font_size, char16_t text, colorRGB color)
{
    ...
    if (!font_available)
    {
        cam->logger->log("Font size not available");
        return;
    }
    ...
    {
        cam->logger->log("Character not available in font");
        return;
    }
