...
namespace pixl::maths
{
    ...
    public:
        ...
        static inline constexpr float Dot(const Vec3f &a, const Vec3f &b) noexcept { return glm::dot(a, b); }
        static inline constexpr Vec3f Cross(const Vec3f &a, const Vec3f &b) noexcept { return glm::cross(a, b); }

        static inline constexpr float Length(const Vec3f &v) noexcept { return glm::length(v); }
        static inline constexpr Vec3f Normalize(const Vec3f &v) noexcept { return glm::normalize(v); }

        static inline constexpr float Distance(const Vec3f &a, const Vec3f &b) noexcept { return glm::distance(a, b); }

        static inline constexpr Vec3f Reflect(const Vec3f &I, const Vec3f &N) noexcept { return glm::reflect(I, N); }
        static inline constexpr Vec3f Refract(const Vec3f &I, const Vec3f &N, float eta) noexcept { return glm::refract(I, N, eta); }

        static inline constexpr Vec3f Lerp(const Vec3f &a, const Vec3f &b, float t) noexcept { return a + (b - a) * t; }

        static inline constexpr float AngleBetween(const Vec3f &a, const Vec3f &b) noexcept
        {
        ...
        static inline constexpr Mat4f Translate(const Vec3f &t) noexcept { return glm::translate(Mat4fidentity, t); }
        static inline constexpr Mat4f Scale(const Vec3f &s) noexcept { return glm::scale(Mat4fidentity, s); }
        static inline constexpr Mat4f Rotate(float angleRad, const Vec3f &axis) noexcept { return glm::rotate(Mat4fidentity, angleRad, axis); }

        static inline constexpr Mat4f LookAt(const Vec3f &eye, const Vec3f &center, const Vec3f &up) noexcept
        {
            return glm::lookAt(eye, center, up);
        }

        static inline constexpr Mat4f Perspective(float fovY, float aspect, float nearZ, float farZ) noexcept
        {
            return glm::perspective(fovY, aspect, nearZ, farZ);
        }

        static inline constexpr Mat4f Ortho(float left, float right, float bottom, float top, float nearZ, float farZ) noexcept
        {
