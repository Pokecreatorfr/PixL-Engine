...
void lifebar(camera* cam, int* w , int* h , int* x , int* y , std::vector<SDL_Texture*>* textures, FontsRenderer* font_renderer  , gui_param param)
{
    if (param.visible)
    {
        ...
        SDL_RenderCopy(cam->renderer, textures->at(0), NULL, &dest2);
    }
    return;
}
...
void caminfo(camera* cam, int* w , int* h , int* x , int* y , std::vector<SDL_Texture*>* textures, FontsRenderer* font_renderer  , gui_param param)
{
    if (param.visible)
    {
        std::string xstring = "X:" + std::to_string(cam->position.x);
        std::string ystring = "Y:" + std::to_string(cam->position.y);
        std::string zoomstring = "Zoom:" + std::to_string(cam->zoom);
        ...
        if(textures->size() == 0)
        {
            SDL_Texture* render_texture = SDL_CreateTexture(cam->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, 310, 34);
            textures->push_back(render_texture);
        }
        SDL_SetRenderTarget(cam->renderer, textures->at(0));
        ...
        SDL_SetRenderTarget(cam->renderer, NULL);
        SDL_RenderCopy(cam->renderer, textures->at(0), NULL, &dest);
    }
    return;
}
